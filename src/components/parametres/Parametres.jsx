...
import { Plus, X, Check, ChevronRight, MapPin, Phone, User, School, BookOpen, Users, GraduationCap, Briefcase, Heart, DollarSign, ClipboardCheck, FileText, Camera } from 'lucide-react';
...
const SECTIONS = [
  ...
  { id: 'photos',      label: 'Photos',      icon: Camera,         color: 'text-pink-600'   },
];

// ── Photo Manager ─────────────────────────────────────────────────────────────
function SchoolPhotoManager({ schoolId, ht }) {
  const [photos, setPhotos] = React.useState([]);
  const [loading, setLoading] = React.useState(true);
  const [editingCaption, setEditingCaption] = React.useState(null);
  const [captionText, setCaptionText] = React.useState('');

  React.useEffect(() => {
    if (!schoolId) return;
    getDocs(collection(db, 'schools', schoolId, 'schoolPhotos')).then(snap => {
      setPhotos(snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (a.uploadedAt||'').localeCompare(b.uploadedAt||'')));
      setLoading(false);
    });
  }, [schoolId]);
  ...
  const saveCaption = async (photo) => {
    await updateDoc(doc(db, 'schools', schoolId, 'schoolPhotos', photo.id), { caption: captionText });
    setPhotos(p => p.map(x => x.id === photo.id ? { ...x, caption: captionText } : x));
    setEditingCaption(null);
  };

  if (loading) return <p className="text-center text-sm text-gray-400 py-4">Chargement...</p>;
  if (photos.length === 0) return (
    <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
      <p className="text-4xl mb-3">📷</p>
      <p className="text-gray-500 text-sm">{ht ? 'Pa gen foto ankò.' : 'Aucune photo pour le moment.'}</p>
    </div>
  );
  return (
      ...
      {photos.map(photo => (
        <div key={photo.id} className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className="relative">
            ...
            <button onClick={() => deletePhoto(photo)} className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-7 h-7 flex items-center justify-center text-xs shadow hover:bg-red-600">✕</button>
          </div>
          <div className="p-2">
            {editingCaption === photo.id ? (
              <div className="flex gap-1">
                <input autoFocus type="text" value={captionText} onChange={e => setCaptionText(e.target.value)} className="flex-1 px-2 py-1 border rounded-lg text-xs" placeholder={ht ? 'Lejand...' : 'Légende...'} />
                <button onClick={() => saveCaption(photo)} className="bg-green-500 text-white px-2 py-1 rounded-lg text-xs">✓</button>
                <button onClick={() => setEditingCaption(null)} className="bg-gray-200 px-2 py-1 rounded-lg text-xs">✕</button>
              </div>
            ) : (
              <button onClick={() => { setEditingCaption(photo.id); setCaptionText(photo.caption || ''); }} className="w-full text-left text-xs text-gray-500 hover:text-gray-700 truncate">
                {photo.caption || <span className="italic text-gray-300">{ht ? 'Ajoute yon lejand...' : 'Ajouter une légende...'}</span>}
              </button>
            )}
          </div>
        </div>
...
export default function Parametres() {
  ...
  const SECTION_LABELS = ht ? {
    ...
    personnel: 'Pèsonèl', viescolaire: 'Lavi Lekòl', finances: 'Finans', photos: 'Foto',
  } : {};
  ...
  const checks = [
    ...
    { label: ht?'Omwen 1 sik':'Au moins 1 cycle',         done: !!val('schoolType','schoolType'),                                       section:'structure'  },
    { label: ht?'Omwen 1 klas':'Au moins 1 classe',       done: classes.length>0,                                                       section:'classes'    },
    ...
    { label: ht?'Foto lekòl':"Photos de l'école",         done: false,                   optional:true,                                section:'photos'     },
  ];
  ...
  const inputCls = 'w-full px-4 py-3 border rounded-xl text-base';
      ...
      <div className="flex-1 pt-4 sm:pt-6 pr-4 sm:pr-6 pl-4 lg:pl-0 pb-6 lg:mt-0 mt-12 max-w-2xl">
        ...
        {activeSection==='identite' && (<div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className={labelCls}>{ht?"Komun":"Commune"} <span className="text-red-400">*</span></label>
                <select value={val('commune','commune')} onChange={e=>set('commune',e.target.value)} disabled={!selectedDept} className={`${inputCls} disabled:bg-gray-100`}>
            ...
            <div>
              ...
              {val('gpsLat','gpsLat')&&(
                <div className="mt-3 rounded-xl overflow-hidden border h-40">
                  <iframe title="map" width="100%" height="100%" frameBorder="0" src={`https://www.openstreetmap.org/export/embed.html?bbox=${parseFloat(val('gpsLng','gpsLng'))-0.005}%2C${parseFloat(val('gpsLat','gpsLat'))-0.003}%2C${parseFloat(val('gpsLng','gpsLng'))+0.005}%2C${parseFloat(val('gpsLat','gpsLat'))+0.003}&layer=mapnik&marker=${val('gpsLat','gpsLat')}%2C${val('gpsLng','gpsLng')}`}/>
                </div>
              )}
            </div>
          </div>
          <div className="bg-white rounded-2xl shadow-lg p-5 space-y-4">
            ...
            <div><label className={labelCls}>WhatsApp</label><input type="tel" value={val('whatsapp','whatsapp')} onChange={e=>set('whatsapp',e.target.value)} className={inputCls} placeholder="+509 XXXX XXXX"/></div>
            <div><label className={labelCls}>{ht?"Sit entènèt":"Site web"}</label><input type="url" value={val('website','website')} onChange={e=>set('website',e.target.value)} className={inputCls} placeholder="https://"/></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className={labelCls}>Facebook</label><input type="text" value={val('facebook','facebook')} onChange={e=>set('facebook',e.target.value)} className={inputCls} placeholder="@monecole"/></div>
              <div><label className={labelCls}>Instagram</label><input type="text" value={val('instagram','instagram')} onChange={e=>set('instagram',e.target.value)} className={inputCls} placeholder="@monecole"/></div>
            </div>
          </div>
        ...
        {activeSection==='directeur' && (<div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className={labelCls}>{ht?"Telefòn":"Téléphone"}</label><input type="tel" value={val('directorPhone','directorPhone')} onChange={e=>set('directorPhone',e.target.value)} className={inputCls} placeholder="+509 XXXX XXXX"/></div>
              <div><label className={labelCls}>Email</label><input type="email" value={val('directorEmail','directorEmail')} onChange={e=>set('directorEmail',e.target.value)} className={inputCls}/></div>
            </div>
          </div>
          <button onClick={saveSettings} className="w-full bg-socrates-blue text-white py-4 rounded-xl font-semibold text-lg">{ht?'Anrejistre':'Sauvegarder'}</button>
        </div>)}
              ...
              {val('adminStaff','adminStaff',[]).map((s,i)=>(
                <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex-1">
                    ...
                    {s.annualSalary>0&&<p className="text-xs text-green-600 font-medium">HTG {parseFloat(s.annualSalary).toLocaleString()} / {ht?'an':'an'}<span className="text-gray-400 font-normal"> • HTG {(parseFloat(s.annualSalary)/10).toLocaleString()} / {ht?'mwa':'mois'}</span></p>}
                  </div>
        ...
        {activeSection==='finances' && (<div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-5 space-y-4">
            <div>
              <h3 className="font-semibold text-gray-800 mb-1">💳 {ht?'Metòd Peman':'Méthodes de Paiement'}</h3>
              <p className="text-xs text-gray-400">{ht?'Ranpli sèlman metòd ou itilize. Paran ak sponsor ap wè yo nan pòtal yo.':'Remplissez uniquement les méthodes que vous utilisez. Parents et sponsors les verront dans leur portail.'}</p>
            </div>
            <div className="space-y-3">
              {[
                {key:'moncash', label:'MonCash', icon:'📱', placeholder:'Nimewo MonCash — ex: 509 XXXX XXXX'},
                {key:'natcash', label:'Natcash', icon:'📲', placeholder:'Nimewo Natcash — ex: 509 XXXX XXXX'},
                {key:'bank',    label:ht?'Depò Labank':'Dépôt bancaire', icon:'🏦', placeholder:'Labank + Nimewo kont — ex: BUH / 0012345'},
                {key:'zelle',   label:'Zelle (USA)', icon:'💸', placeholder:'Telefòn oswa email Zelle'},
                {key:'paypal',  label:'PayPal', icon:'🅿️', placeholder:'Email PayPal'},
              ].map(m => {
                const pm = val('paymentMethods','paymentMethods',{});
                return (
                  <div key={m.key} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <input type="text"
                        value={pm[m.key] || ''}
                        onChange={e => { const p = {...pm}; if(e.target.value.trim()) p[m.key]=e.target.value; else delete p[m.key]; set('paymentMethods',p); }}
        ...
        {/* ═══ PHOTOS ═══ */}
        {activeSection==='photos' && (<div className="space-y-5">
          <h2 className="text-xl font-bold text-gray-800">📸 {ht?'Galri Foto':'Galerie Photos'}</h2>
          <p className="text-sm text-gray-500">{ht?"Foto sa yo pral parèt nan pwofil piblik lekòl la nan anyè a.":"Ces photos apparaîtront dans le profil public de l'école dans l'annuaire."}</p>
          <div className="bg-white rounded-2xl shadow-lg p-5">
            <label className={`flex items-center justify-center gap-3 border-2 border-dashed border-pink-200 text-pink-600 py-4 rounded-xl text-sm font-medium cursor-pointer hover:bg-pink-50 transition ${formData.photoUploading?'opacity-50 pointer-events-none':''}`}>
              <Camera size={20}/>
              {formData.photoUploading?(ht?'⏳ Ap telechaje...':'⏳ Téléchargement...'):(ht?'📷 Ajoute foto':'📷 Ajouter des photos')}
            ...
            <p className="text-xs text-gray-400 mt-2 text-center">JPG, PNG — max 2MB {ht?'pa foto':'par photo'}</p>
          </div>
          <SchoolPhotoManager schoolId={school?.id} ht={ht}/>
        </div>)}
