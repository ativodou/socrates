...
export default function Classes({ onOpenModal, onViewClass }) {
  ...
  const totalStudentsEnrolled = new Set(students.flatMap(s => (s.enrolledClasses || []).map(cId => `${s.id}-${cId}`))).size;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 text-center">
          ...
          <p className="text-xs text-gray-500">{teacherLabel}</p>
        </div>
        <div className="bg-white rounded-xl shadow-lg p-4 text-center">
          <p className="text-2xl font-bold text-purple-600">{gradingPeriods.length}</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          {!prescoOnly && <button onClick={() => onOpenModal('period')} className="flex-1 sm:flex-none bg-gray-100 text-gray-700 px-4 py-3 rounded-xl font-medium flex items-center justify-center gap-2"><Calendar size={18} />Périodes</button>}
          <button onClick={() => onOpenModal('class')} className="flex-1 sm:flex-none bg-socrates-blue text-white px-4 py-3 rounded-xl font-medium flex items-center justify-center gap-2"><Plus size={18} />Ajouter</button>
        </div>
      </div>
      ...
      {gradingPeriods.length > 0 && (
        <div className="bg-white rounded-xl shadow-lg p-4">
          <h4 className="font-medium text-gray-700 mb-3 flex items-center gap-2"><Calendar size={16} className="text-purple-500" /> Périodes</h4>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          return (
            <div key={cls.id} className="bg-white rounded-xl shadow-lg p-5 flex flex-col">
              <div className="flex items-start justify-between mb-3">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-socrates-navy to-socrates-blue text-white flex items-center justify-center">
                  <BookOpen size={24} />
                </div>
              ...
              <div className="mt-3 pt-3 border-t">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-500 flex items-center gap-1"><Users size={14} /> {studentCount} {adult ? 'étudiant' : 'élève'}{studentCount !== 1 ? 's' : ''}</span>
                  {cls.books?.length > 0 && <span className="text-gray-400">{cls.books.length} livre{cls.books.length > 1 ? 's' : ''}</span>}
                </div>
                {capacityPct !== null && (
                  <div className="mt-2">
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-gray-400">Capacité</span>
                      <span className={capacityPct >= 90 ? 'text-red-500 font-medium' : capacityPct >= 70 ? 'text-orange-500' : 'text-gray-500'}>{studentCount}/{cls.maxCapacity}</span>
                    </div>
                    <div className="w-full bg-gray-100 rounded-full h-1.5">
                      <div className={`h-1.5 rounded-full transition-all ${capacityPct >= 90 ? 'bg-red-500' : capacityPct >= 70 ? 'bg-orange-400' : 'bg-socrates-blue'}`} style={{ width: `${Math.min(capacityPct, 100)}%` }} />
                    </div>
                  </div>
                )}
              </div>

              <button onClick={() => onViewClass(cls)} className="w-full mt-4 bg-gray-100 text-gray-700 py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2 hover:bg-gray-200 transition">
                <ClipboardList size={16} />Voir détails
              </button>
            </div>
          );
        })}
        {classes.length === 0 && (
          <div className="col-span-full text-center py-12 text-gray-500 bg-white rounded-xl shadow-lg">
            <BookOpen size={48} className="mx-auto mb-4 opacity-50" /><p>Aucune {prescoOnly ? 'section' : 'classe'}</p>
          </div>
        )}
      </div>
    </div>
