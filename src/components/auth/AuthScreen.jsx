...
export default function AuthScreen() {
  const { user, handleRegister, handleLogin, handleGoogleSignIn, handleGoogleSetup } = useSchool();
  ...
  const onGoogleSetup = async (e) => {
    e.preventDefault();
    setError('');
    try { await handleGoogleSetup(formData); }
    catch (err) { setError(err.message); }
  };
  ...
  // Google user authenticated but no school doc yet — show school setup
  if (user) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-socrates-navy via-socrates-blue to-blue-400 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
          <div className="p-8 text-center bg-gradient-to-r from-socrates-navy to-socrates-blue text-white">
            <img src="/owl-icon.svg" alt="SOCRATES" className="w-16 h-16 rounded-full mx-auto mb-3 object-contain" />
            <h1 className="text-2xl font-display">SOCRATES</h1>
            <p className="text-blue-200 text-sm mt-1">
              {parentLang === 'ht' ? 'Bon vini, ' : 'Bienvenue, '}{user.displayName || user.email} !
            </p>
          </div>
          <div className="p-6">
            <h2 className="text-lg font-bold text-gray-800 mb-1">
              {parentLang === 'ht' ? 'Konfigire lekòl ou a' : 'Configurer votre école'}
            </h2>
            <p className="text-sm text-gray-500 mb-5">
              {parentLang === 'ht' ? 'Antre enfòmasyon lekòl ou a pou kontinye.' : 'Entrez les informations de votre école pour continuer.'}
            </p>
            {error && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl mb-4 text-sm">{error}</div>}
            <form onSubmit={onGoogleSetup} className="space-y-4">
              <input type="text" placeholder={parentLang === 'ht' ? 'Non lekòl la *' : "Nom de l'école *"} required
              ...
              <input type="text" placeholder={parentLang === 'ht' ? 'Non direktè a *' : 'Nom du directeur *'} required
                value={formData.directorName || ''} onChange={e => setFormData({ ...formData, directorName: e.target.value })}
                className="w-full px-4 py-3 border rounded-xl text-base" />
              <input type="tel" placeholder={parentLang === 'ht' ? 'Telefòn' : 'Téléphone'}
                value={formData.phone || ''} onChange={e => setFormData({ ...formData, phone: e.target.value })}
                className="w-full px-4 py-3 border rounded-xl text-base" />
              <button type="submit" className="w-full bg-socrates-blue text-white py-3 rounded-xl font-semibold">
                {parentLang === 'ht' ? 'Kòmanse →' : 'Commencer →'}
              </button>
            </form>
          </div>
        </div>
          ...
          {authMode === 'login' && (
            <div className="space-y-4">
              <button type="button" onClick={onGoogleSignIn} disabled={googleLoading}
                className="w-full flex items-center justify-center gap-3 border border-gray-200 py-3 rounded-xl font-semibold text-gray-700 hover:bg-gray-50 transition disabled:opacity-60">
                ...
                {googleLoading ? '...' : (parentLang === 'ht' ? 'Kontinye ak Google' : 'Continuer avec Google')}
              </button>
              <div className="flex items-center gap-3">
                <div className="flex-1 border-t border-gray-200" />
                <span className="text-xs text-gray-400">{parentLang === 'ht' ? 'oswa' : 'ou'}</span>
                <div className="flex-1 border-t border-gray-200" />
              </div>
          ...
          {authMode === 'register' && (
            <div className="space-y-4">
              <button type="button" onClick={onGoogleSignIn} disabled={googleLoading}
                className="w-full flex items-center justify-center gap-3 border border-gray-200 py-3 rounded-xl font-semibold text-gray-700 hover:bg-gray-50 transition disabled:opacity-60">
                <svg width="20" height="20" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
                {googleLoading ? '...' : (parentLang === 'ht' ? 'Enskripsyon ak Google' : "S'inscrire avec Google")}
              </button>
              <div className="flex items-center gap-3">
                <div className="flex-1 border-t border-gray-200" />
                <span className="text-xs text-gray-400">{parentLang === 'ht' ? 'oswa' : 'ou'}</span>
